...
function MostRatedCourseCard({ course }) {
  let token = readToken();
      ...
      <Link href={`/course-details/${courseCode}`} legacyBehavior passHref>
        <a className="shadow rounded-2" style={{ display: 'inline-block', overflow: "hidden", textDecoration: 'none', width: '230px' }}>
          <Card className="border-0 text-black">
            <Card.Body className="pa-0 d-flex flex-column justify-content-between" style={{ height: "210px" }}>
              <Card.Img variant="top" src={imageUrl} style={{ objectFit: 'cover', height: "130px" }} />
...
export default MostRatedCourseCard;
